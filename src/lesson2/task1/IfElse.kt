...
fun triangleKind(a: Double, b: Double, c: Double): Int {
    val angleAlpha = acos((sqr(a) + sqr(b) - sqr(c)) / (2 * a * b)) * 180 / PI
    val angleBeta = acos((sqr(a) + sqr(c) - sqr(b)) / (2 * a * c)) * 180 / PI
    val angleGamma = acos((sqr(c) + sqr(b) - sqr(a)) / (2 * b * c)) * 180 / PI
    return when {
        angleAlpha < 90 && angleBeta < 90 && angleGamma < 90 -> 0
        angleAlpha > 90 || angleBeta > 90 || angleGamma > 90 -> 2
        a + b < c || a + c < b || b + c < a -> -1
        else -> 1
    }
}
